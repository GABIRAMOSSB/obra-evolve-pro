...
export function NfeRateioDialog({ open, onOpenChange, companyId, item, nota, obras, onSaved }: Props) {
 ...
 <span className={Math.abs(restante) < 0.001 ? "text-success font-medium" : restante < 0 ? "text-destructive font-medium" : "text-warning-foreground font-medium"}>
 ...
 <SelectTrigger className="w-full min-w-0"><SelectValue placeholder={r.obra_id ? "Composição…" : "Defina obra"} /></SelectTrigger>
 <SelectContent>
 {compRows.map((b) => (
