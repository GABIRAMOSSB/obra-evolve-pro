/**
 * Módulo Subempreiteiros — server functions.
 * Triggers no banco cuidam de: cap de quantidade, bloqueio pós-assinatura
 * e validações de pagamento.
 */
...
export const listSubempreiteiros = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const companyId = await resolveCompanyId(supabase, context.userId as string);
    const { data, error } = await supabase
      .from("subempreiteiros").select("*").eq("company_id", companyId).order("razao_social");
    if (error) throw new Error(error.message);
    return data ?? [];
  });
...
export const saveSubContrato = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const companyId = await resolveCompanyId(supabase, context.userId as string);
    const payload: Record<string, unknown> = {
      ...data, company_id: companyId, created_by: context.userId,
    };
    if (data.id) {
      const { id, ...patch } = payload;
      const { error } = await supabase
...
// Importa serviços a partir do orçamento mais recente da obra do contrato
export const importarServicosDoOrcamento = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: contrato } = await supabase
      .from("sub_contratos").select("obra_id").eq("id", data.contrato_id)
      .eq("company_id", companyId).single();
    if (!contrato) throw new Error("Contrato não encontrado.");
    // versão mais recente do orçamento
    const { data: versao } = await supabase
      .from("orcamento_versoes").select("id,status,numero_versao")
      .eq("obra_id", contrato.obra_id).eq("company_id", companyId)
      .order("numero_versao", { ascending: false }).limit(1).maybeSingle();
    if (!versao) throw new Error("Obra não possui orçamento.");
    const { data: itens } = await supabase
      .from("orcamento_itens")
      .select("id,item_codigo,descricao,unidade,qtd_contratada,valor_unitario_cents,ordem,tipo")
      .eq("versao_id", versao.id).order("ordem", { ascending: true });
    if (!itens || itens.length === 0) throw new Error("Orçamento vazio.");
    const payload = itens
      .filter((it: Record<string, unknown>) => it.tipo !== "grupo")
      .map((it: Record<string, unknown>, idx: number) => ({
        company_id: companyId,
        contrato_id: data.contrato_id,
        codigo: (it.item_codigo as string | null) ?? null,
        descricao: (it.descricao as string) ?? "—",
        unidade: (it.unidade as string | null) ?? null,
        qtd_contratada: Number(it.qtd_contratada ?? 0),
        preco_unitario: 0,
        preco_venda_cliente: Number(it.valor_unitario_cents ?? 0) / 100,
        ordem: idx,
      }));
    if (payload.length === 0) throw new Error("Nenhum item elegível para importação.");
    const { error } = await supabase.from("sub_contrato_servicos").insert(payload);
    if (error) throw new Error(error.message);
...
export const listSubMedicoes = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabase
      ...
      .eq("company_id", companyId).order("data_medicao", { ascending: false });
    if (error) throw new Error(error.message);
    return data ?? [];
  });
...
export const createSubMedicao = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: last } = await supabase
      .from("sub_medicoes").select("numero").eq("contrato_id", data.contrato_id)
      .order("numero", { ascending: false }).limit(1).maybeSingle();
    const numero = ((last?.numero as number | undefined) ?? 0) + 1;
    const { data: med, error } = await supabase.from("sub_medicoes").insert({
      company_id: companyId,
      contrato_id: data.contrato_id,
      numero,
    ...
    if (servicos && servicos.length) {
      const itens = servicos.map((s: Record<string, unknown>) => ({
        company_id: companyId,
        medicao_id: med.id,
      ...
      const { error: eIt } = await supabase.from("sub_medicao_itens").insert(itens);
      if (eIt) throw new Error(eIt.message);
    }
    return { id: med.id as string, numero };
  });
...
export const recalcSubMedicao = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: {
    id: string; ret_inss?: number; ret_iss?: number; ret_irrf?: number; ret_outras?: number;
    observacoes?: string | null; responsavel_conferencia?: string | null;
  }) => z.object({
    id: z.string().uuid(),
  ...
  .handler(async ({ data, context }) => {
    ...
    const companyId = await resolveCompanyId(supabase, context.userId as string);
    const { data: itens } = await supabase
      .from("sub_medicao_itens").select("valor")
      .eq("medicao_id", data.id).eq("company_id", companyId);
    const bruto = (itens ?? []).reduce(
      (s: number, r: Record<string, unknown>) => s + Number(r.valor ?? 0), 0,
    );
    ...
    const { error } = await supabase.from("sub_medicoes").update({
...
export const listSubPagamentos = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabase
      ...
      .eq("company_id", companyId).order("data_pagamento", { ascending: false });
    if (error) throw new Error(error.message);
    return data ?? [];
  });
...
export const getSubDashboard = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [ctRes, svcRes, medRes, pgRes] = await Promise.all([
      supabase.from("sub_contratos").select("id,valor_maximo,status,obra_id").eq("company_id", companyId),
      supabase.from("sub_contrato_servicos").select("qtd_contratada,qtd_executada,preco_unitario,preco_venda_cliente,contrato_id").eq("company_id", companyId),
      supabase.from("sub_medicoes").select("valor_bruto,valor_liquido,status,contrato_id").eq("company_id", companyId),
      supabase.from("sub_pagamentos").select("valor_pago,medicao_id").eq("company_id", companyId),
    ]);
    const contratos = (ctRes.data ?? []) as Array<Record<string, unknown>>;
    const svcs = (svcRes.data ?? []) as Array<Record<string, unknown>>;
    const meds = (medRes.data ?? []) as Array<Record<string, unknown>>;
    const pgs = (pgRes.data ?? []) as Array<Record<string, unknown>>;
    ...
    const total_aprovado = meds
      .filter((m) => ["assinada", "liberada_pagamento", "paga"].includes(m.status as string))
