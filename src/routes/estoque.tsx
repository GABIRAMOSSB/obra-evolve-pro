...
function EstoquePage() {
  ...
  async function handleSaida() {
    ...
    const { error } = await supabase.from("estoque_movimentos").insert({
      ...
      origem: "manual",
      quantidade: qtd,
      valor_unitario: (Number(formValor) || (() => { const e = movimentos.filter(m => m.insumo_id === formInsumo && m.tipo === "entrada"); const tq = e.reduce((a, m) => a + Number(m.quantidade), 0); const tv = e.reduce((a, m) => a + Number(m.valor_total), 0); return tq > 0 ? tv / tq : 0; })()),
      valor_total: qtd * (Number(formValor) || (() => { const e = movimentos.filter(m => m.insumo_id === formInsumo && m.tipo === "entrada"); const tq = e.reduce((a, m) => a + Number(m.quantidade), 0); const tv = e.reduce((a, m) => a + Number(m.valor_total), 0); return tq > 0 ? tv / tq : 0; })()),
